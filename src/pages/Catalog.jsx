          ...
          return (
            <div key={p.id} className="border rounded-xl overflow-hidden font-poppins tracking-tighter">
              ...
              <div className="p-3 font-poppins tracking-tighter">
                <div className="text-xs text-slate-500 mb-1 tracking-tighter">{v?.name || "—"}</div>
                <div className="text-sm font-medium line-clamp-1 tracking-tighter">{p.name}</div>
                <div className="text-sm font-semibold tracking-tighter">{currency(p.price)}</div>
                {etaLabel && (
                  <div className="mt-0.5 text-[11px] text-slate-500 tracking-tighter">{etaLabel}</div>
                )}
                <div className="mt-2 flex items-center gap-2 tracking-tighter">
                  <Button size="sm" className="flex-1 font-poppins tracking-tighter" onClick={() => go("product", { id: p.id })}>
                    View
                  </Button>
                  <Button size="icon" variant="outline" className="font-poppins tracking-tighter" onClick={() => addToCart(p.id)}>
