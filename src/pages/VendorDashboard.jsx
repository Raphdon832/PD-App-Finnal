...
export default function VendorDashboard({
  ...
  const ensureVendorId = () => {
    ...
    if (vid && !lockedVendorIdRef.current) lockedVendorIdRef.current = String(vid);
  ...
  const onImport = async (e) => {
    ...
    if (added > 0) {
      if (typeof window !== 'undefined' && window.toast) window.toast(`Imported ${added} item(s)`);
    }
  };
    ...
    <div className="space-y-6 px-2 sm:px-0">
      <section className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          {showingInit && (
            <Badge variant="secondary" className="text-xs">Initializing vendor…</Badge>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="md:col-span-2 flex flex-col sm:flex-row items-center gap-3">
            <label className="w-full sm:w-auto">
              <span className="block mb-1 text-blue-600 underline cursor-pointer text-sm">Choose photo</span>
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <input type="file" accept=".csv,.xml" multiple onChange={onImport} className="w-full sm:w-auto" />
      ...
      <section className="space-y-2">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <div className="text-xs text-slate-500">{products.length} item(s)</div>
        </div>
          ...
          {products.map((p) => (
            <div key={p.id} className="border rounded-xl overflow-hidden">
              ...
              <div className="p-3 space-y-1">
                ...
                <div className="text-xs text-slate-500">₦{Number(p.price || 0).toLocaleString()} • Stock {p.stock}</div>
                <div className="pt-1 flex flex-col sm:flex-row gap-2">
                  <Button variant="ghost" size="sm" onClick={() => navigator?.clipboard?.writeText(p.id)}>
                    Copy ID
                  </Button>
