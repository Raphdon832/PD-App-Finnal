          ...
          <Card key={ci.id}>
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500">{currency(p?.price)} • Qty</div>
                <div className="flex items-center gap-2 mt-1">
                  <Input type="number" className="w-20" value={ci.qty} onChange={(e)=>setQty(ci.id, Number(e.target.value||1))}/>
                  <Button variant="ghost" size="icon" onClick={()=>removeLine(ci.id)}><Trash2 className="h-4 w-4"/></Button>
                </div>
              </div>
              <div className="font-semibold">{currency((p?.price||0) * ci.qty)}</div>
            </CardContent>
          </Card>
        );
      })}
